...
$strSQL7 ="SELECT * FROM  criteria Left join student ON (criteria.std_id=student.std_id) WHERE criteria.regis_section = '".$_GET["sub_id"]."'  ";
...
<ol class="breadcrumb">
    <li class="breadcrumb-item active"><h4>ผลการเรียน&nbsp;/&nbsp;แก้ไขคะแนนรวม</h4></li>
...
<form name="frmMain" method="post" action="score_update.php?sub_id=<?=$objResult2["sub_id"]?>">
   ...
   while($objResult7 = mysqli_fetch_array($objQuery7))
{
  ...
  ?>
  <input type="hidden" name="cri_id[]" value="<?php
  
  echo($objResult7["cri_id"]);
...
$objResult14 = mysqli_fetch_array($objQuery14);
 ...
 <?php
   ...
   $score10 = mysqli_fetch_array(mysqli_query($con,"SELECT cn_score FROM criteria WHERE std_id = '".$objResult7["std_id"]."' and regis_section = '".$_GET["sub_id"]."' "));
 //echo $score10[0]; 

  ?>

  
  <td align=center><input type="text" name="std_name[]" value="<?php echo $score10[0]; ?>" size="2" onKeyUp="if(isNaN(this.value)){ alert('กรุณากรอกตัวเลข'); this.value='';}" required></td>
...
<td align=center>
  ...
  <?php
$score12 = mysqli_fetch_array(mysqli_query($con,"SELECT sum(score) FROM work_score WHERE std_id = '".$objResult7["std_id"]."' and regis_section = '".$_GET["sub_id"]."' "));
 echo $score12[0]; 
...
?>
  ...
  <center>
 <a href="score_total.php?sub_id=<?=$objResult2["sub_id"]?>"><button type="button" class="btn btn-primary"> ย้อนกลับ</button></a>
